...
export default class SuiteCard extends Component {
    ...
    showReviewsFn = (room) => {
        return (<BedroomModal isSingle={false} title={room.roomCode} room={room} handleActivateViewSuite={this.viewSuiteRooms}/>);
    }
    ...
    handleSuiteFavorited = () => {
        ...
        this.props.handleFavoritedSuite({suiteCode: this.state.name, buildingName: this.props.suite.buildingName,favorited: favorited})
    ...
    computeSuiteStats = (suite) => {
        var no = 0, noise = 0, size = 0, noBeds = 0, noReviews = 0;
        var reviews = [];
        ...
        return {noise: noise, size: size, noBeds: noBeds, noReviews: noReviews, previewReview: preview};
    }
    ...
    render() {
        return (
            <div className='suite-card'>
                ...
                <div
                    className='card-right-side-suite col-md-7' onClick={this.activateModal}
                >

                    {/* room number and bookmark icon */}
                    <div className='card-title-container'>
                        <h5 className='card-title'>
                            {this.props.suite.suiteCode}
                        </h5>
                    </div>
                    ...
                    <p className='card-review-quotes'>"{this.state.suiteStats.previewReview}"</p>
                    <h1 className='card-subtext'>{this.state.suiteStats.noReviews} reviews</h1>
                </div>
                <div className='favorite-suite' onClick={this.handleSuiteFavorited}>
                    {!this.state.favorited ? (<FaRegBookmark style={{ color: '#0053c5', fontSize: '30px' }} />): (<FaBookmark style={{ color: '#0053c5', fontSize: '30px' }} />)}
                    <div className='favorited-inside'>
                        {this.state.favoritedInside ? (<BsDot style={{ color: '#0053c5', fontSize: '30px' }} />) : ('')}
                    </div>
                </div>
                ...
                >
                    ...
                    (<BedroomModal isSingle={true} title={this.props.suite.suiteRooms[0].roomCode} room={this.props.suite.suiteRooms[0]} handleActivateViewSuite={this.viewSuiteRooms}/>)
                    :
                    (
                    <>
                    {!this.state.showReviews ? (<SuiteModal title={this.props.suite.suiteCode} suiteStats={this.state.suiteStats} rooms={this.props.suite.suiteRooms} handleFavorited={this.handleRoomFavorited} handleActivateReview={this.activateReview}/>): (this.showReviewsFn(this.state.showReviewRoomInfo))}
                    </>
                    )}
