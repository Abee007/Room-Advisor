    ...
    return (
      <div className='bedroom-modal'>
        <div className='bedroom-modal-header'>
          <div className={!this.props.isSingle ? 'bedroom-modal-backarrow': 'bedroom-modal-backarrow zero-opacity'} onClick={this.goBackToSuiteView}>
            <IoIosArrowBack style={{ color: '#0053c5', fontSize: '30px' }} />
          </div>
          
          <h4 className='bedroom-modal-title'>{this.props.title}</h4>
          <div className={!this.props.isSingle ? 'bedroom-badge-container' : 'bedroom-badge-container is-single'}>
            <div className='pill'><Badge pill bg='primary'>{numberToAcronym(this.props.room.meta.noBeds)}</Badge></div>
            <div className='pill'><Badge pill bg='secondary'>Noise: {this.props.room.meta.noise} </Badge></div>
            <div className='pill'><Badge pill bg='info'> Size: {this.props.room.meta.size} </Badge></div>
          </div>
          <div className={!this.props.isSingle ? 'favorite-room-modal': 'favorite-room-modal fav-room-standalone'} onClick={this.toggleFavorited}>
            {!this.props.room.meta.favorited ? (<FaRegBookmark style={{ color: '#0053c5', fontSize: '25px' }} />): (<FaBookmark style={{ color: '#0053c5', fontSize: '25px' }} />)}
          </div>
        </div>
