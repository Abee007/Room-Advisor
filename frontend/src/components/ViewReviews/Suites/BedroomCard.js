...
export default class BedroomCard extends Component {
  ...
  toggleFavorited = () => {
    ...
    this.props.handleFavorited({roomCode: this.props.room.roomCode, favorited: favorited});
  ...
  render () {
    return (
      <div className='card'>
        ...
        <div className='room-card-right-side col-md-7' onClick={this.activateReview}>
          ...
          <div className='room-card-badge-container'>
            <Badge pill bg='primary'>{numberToAcronym(this.props.room.meta.noBeds)}</Badge>
            <Badge pill bg='secondary'>Noise: {this.props.room.meta.noise}</Badge>
            <Badge pill bg='info'>Size: {this.props.room.meta.size}</Badge>
          </div>

          <p className='room-card-review-quotes'> "It's tinyyyyyyy and..."</p>
          <h1 className='card-subtext'>{this.props.room.meta.roomReviews.length} reviews</h1>
        </div>
        <div className='favorite-room' onClick={this.toggleFavorited}>
            {!this.state.favorited ? (<FaRegBookmark style={{ color: '#0053c5', fontSize: '25px' }} />): (<FaBookmark style={{ color: '#0053c5', fontSize: '25px' }} />)}
        </div>
      </div>
    )
  }
